...
All notable changes to the MCOS project will be documented in this file.
...
### Changed
- **Project renamed** from MCIS (Museum Collections Information System) to **MCOS (Museum Collections & Operations System)** — the new name reflects the system's actual scope, which includes the Members, Donors, Locations & Loans operational core and the Operations Modules section, not collections information alone
- Global rename of "MCIS" → "MCOS" and "Museum Collections Information System" → "Museum Collections & Operations System" across all documentation (README, High Level Plan, Documentation Plan, schema docs, use case index, and this CHANGELOG, including historical entries)
- Renamed `docs/MCIS_High_Level_Plan.md` → `docs/MCOS_High_Level_Plan.md`, `docs/MCIS_Documentation_Plan.md` → `docs/MCOS_Documentation_Plan.md`, and `docs/MCIS_Documentation_Plan_Critique_v04.md` → `docs/MCOS_Documentation_Plan_Critique_v04.md`
- In-doc repository links updated to `github.com/juren53/MCOS` (README §Contributing, High Level Plan §10) — these resolve once the GitHub repository is renamed
- `docs/MCOS_High_Level_Plan.md` — version bumped to 0.19

### Pending (external, outside this repo)
- Rename the GitHub repository `juren53/MCIS` → `juren53/MCOS` (GitHub auto-redirects the old URL)
- Update sibling repositories (qr-code-generator, tag-writer, audio-tag-writer, HST-Metadata) and any grant/comms materials that reference MCIS
...
### Added
- `docs/MCOS_High_Level_Plan.md` — §4 Collections module: **Scanning Module** — an interface layer between existing, proven scanning software and Collections (not a scanner driver), integrating the pre-existing Tag Writer photo-metadata functions; MCOS does not re-invent functional, well-proven wheels
- `docs/MCOS_High_Level_Plan.md` — §4 Collections module: audio recording (MP3, WAV) attachment with ID3v2.3 archival metadata embedding via the Audio Tag Writer (ATW) functions, mirroring the image metadata workflow
- `docs/MCOS_High_Level_Plan.md` — §4 Collections module: **Audio Ingest Module** — the audio counterpart to the Scanning Module, integrating the pre-existing ATW functions already running in production at the Truman Library
- `docs/MCOS_High_Level_Plan.md` — §11 Prior Work intro: NARA validation paragraph — image and audio files processed at the Truman Library using HPM, Tag Writer, and ATW have been uploaded to and validated in the U.S. National Archives (NARA) Catalog and are currently available online

### Changed
- `docs/MCOS_High_Level_Plan.md` — all seven ExifTool references updated to "ExifTool (and/or Exiv2)" (§4 Collections, §5 IPTC row, §6 tech stack, §11 HPM and Tag Writer)
- `docs/MCOS_High_Level_Plan.md` — version bumped to 0.18
...
### Fixed
- `docs/MCOS_Documentation_Plan.md` — Notes: corrected stale "Community & Sustainability (Phase 4)" to Phase 5, reflecting the earlier Phase 4/5 swap

### Changed
- `docs/MCOS_Documentation_Plan.md` — Phase 1 section heading aligned with the High Level Plan ("Core Framework" → "Core Framework + Basic IA Publishing"); version bumped to 0.8
...
### Added
- `docs/MCOS_High_Level_Plan.md` — §5 new "Grant Alignment" subsection: maps six funder priorities (public access, rights, data stewardship, interoperability, sustainability, mission alignment) to concrete MCOS features, with a candid note that single-developer sustainability is the likeliest review concern
- `docs/MCOS_High_Level_Plan.md` — §11 QR Code Generator prototype: link to the six placeholder IA records added ([archive.org/details/@juren?query=Weston](https://archive.org/details/@juren?query=Weston))

### Changed
- `docs/MCOS_High_Level_Plan.md` — §2 "Simple to Operate" principle reframed around intuitive, easy use for staff/volunteers experienced in museum and archive work, dropping the "high turnover" framing
- `docs/MCOS_High_Level_Plan.md` — §2 "Grant Friendly" principle: subject-verb agreement fixed ("supports" → "support")
- `docs/MCOS_High_Level_Plan.md` and `CHANGELOG.md` — Whitehead Home Museum renamed to Weston Historical Museum throughout (WHM abbreviation unchanged)
- `docs/MCOS_High_Level_Plan.md` — §11 QR Code Generator prototype reworded; count corrected from two to six QR codes; version bumped to 0.16
---

## [2026-06-17-1725] — Correct WHM prototype artifact names (v0.17)

### Changed
- `docs/MCOS_High_Level_Plan.md` — §11 QR Code Generator prototype: artifact names corrected from "ship's bell and bridge clock" to "steamboat bell and anchor bracket and steamboat drum alarm clock", consistent with content files in the qr-code-generator repository; version bumped to 0.17
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — §6 technology stack: Database row updated to "PostgreSQL (multi-user) / SQLite (single-user)"; multi-user and single-user distinctions consolidated into one row; redundant "Single-user option" paragraph removed; version bumped to 0.15
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — §7 Phase 0: advisory board bullet removed; advisory board formation is ongoing and not a Phase 0 gate
- `docs/MCOS_High_Level_Plan.md` — §8 status table: advisory board row removed; version bumped to 0.14
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — Phase 4 and Phase 5 swapped: Data Migration moves to Phase 4, Community & Sustainability moves to Phase 5; pilot institutions need migration tooling before community infrastructure; §5 LIDO reference updated Phase 5 → Phase 4; version bumped to 0.13
- `docs/MCOS_Documentation_Plan.md` — Phase 4 and Phase 5 sections swapped to match High Level Plan; version bumped to 0.7
...
### Added
- `docs/MCOS_High_Level_Plan.md` — §11 Prior Work: QR Code Generator WHM Prototype added; describes origin (Shelly Franklin's suggestion at WHM), two test artifacts (ship's bell, bridge clock), testing findings (printed codes scan faster, sub-2cm viable), and practical trade-offs identified; framed as the direct prototype for the MCOS QR Code Generator module
- `docs/MCOS_High_Level_Plan.md` — §11 intro updated to acknowledge prior work extends to both HSTL and WHM; version bumped to 0.12

---

## [2026-06-17-1253] — Establish QR Code Generator as first standalone MCOS module

### Added
- QR Code Generator — standalone module at [github.com/juren53/qr-code-generator](https://github.com/juren53/qr-code-generator); generates printable QR codes (PNG/PDF) linking physical Museum artifacts to their Internet Archive records; triggered when an `ia_identifier` is assigned to an object following IA publication; prototype developed for WHM (Weston Historical Museum) with test codes for a ship's bell and bridge clock artifact; module is independent of the MCOS core application and can be adopted separately
...
### Added
- `docs/MCOS_Documentation_Plan.md` — QR Code Generator README added to Complete section: standalone module overview at github.com/juren53/qr-code-generator
- `docs/MCOS_Documentation_Plan.md` — User Guide: QR Code Generator added as Phase 1 planned document: label generation workflow, format and size options, batch printing, physical display considerations
- `docs/MCOS_Documentation_Plan.md` — Complete count updated 7 → 8; Planned count updated 15 → 16; version bumped to 0.6
...
### Added
- `docs/MCOS_High_Level_Plan.md` — §4: QR Code Generator added as a standalone module subsection; describes label generation triggered by IA publish, Phase 2 batch support, and links to github.com/juren53/qr-code-generator
- `docs/MCOS_High_Level_Plan.md` — §7 Phase 1: QR Code Generator bullet added after Basic IA Publisher; Phase 1 milestone updated to include printing a QR code label
- `docs/MCOS_High_Level_Plan.md` — §8 Current Status: QR Code Generator module repository added as Complete; version bumped to 0.11
...
### Added
- `README.md` — Modules section added; references the QR Code Generator as the first standalone MCOS module ([juren53/qr-code-generator](https://github.com/juren53/qr-code-generator)); generates printable QR codes linking physical Museum artifacts to their Internet Archive records
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — §11 Prior Work: Truman Home Music Collection added; proof-of-concept project with a live Internet Archive collection at archive.org/details/trumanhomemusic; framed as direct prior art for MCOS's IA publishing model — proves out end-to-end IA workflow (collection setup, item upload, metadata mapping, public access); version bumped to 0.10
...
### Added
- `docs/MCOS_Documentation_Plan.md` — Quick Start Guide added as Phase 1 planned document: first-session walkthrough for new users
- `docs/MCOS_Documentation_Plan.md` — SECURITY.md added as Phase 1 planned document: security disclosure policy and responsible reporting procedure
- `docs/MCOS_Documentation_Plan.md` — Release Notes added as Phase 1 planned document: plain-language per-release summaries for non-technical stakeholders
- `docs/MCOS_Documentation_Plan.md` — Phase 2 Database Schema (Members/Comms) added as Phase 2 planned document: schema not yet drafted

### Changed
- `docs/MCOS_Documentation_Plan.md` — Use Case Library: hardcoded "42 use cases" count removed from description
- `docs/MCOS_Documentation_Plan.md` — Phase 2 Database Schema entry renamed to clarify scope (Locations, Loans, Donors); Members/Comms split into separate planned entry
- `docs/MCOS_Documentation_Plan.md` — Contributing Guide moved from Phase 4 to Phase 1
- `docs/MCOS_Documentation_Plan.md` — Extension Developer Guide: note added that document is contingent on extension framework design
- `docs/MCOS_Documentation_Plan.md` — Phase 2 section heading: "Collections Modules" → "Operational Modules"
- `docs/MCOS_Documentation_Plan.md` — Notes section: updated to acknowledge Members/Comms schema gap
- `docs/MCOS_Documentation_Plan.md` — System Architecture Document: ExifTool image metadata embedding added to description
- `docs/MCOS_Documentation_Plan.md` — User Guide: Collections & Objects: IPTC/EXIF metadata embedding added to description
- `docs/MCOS_Documentation_Plan.md` — Data Migration Guide: IPTC metadata extraction added to batch image import description
- `docs/MCOS_Documentation_Plan.md` — Planned count updated 11 → 15; version bumped to 0.5
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — 12 critique points addressed; version bumped to 0.9:
...
### Added
- `docs/MCOS_High_Level_Plan.md` — new §11 Prior Work & Related Projects documenting three production tools built for the Harry S. Truman Presidential Library: HPM (HSTL Photo Metadata Framework), Tag Writer, and Audio Tag Writer (ATW); each entry covers purpose, key capabilities, and MCOS relevance with repository links
- `docs/MCOS_High_Level_Plan.md` — IPTC added to §5 Standards Alignment table; field mapping proven in HPM and Tag Writer

### Changed
- `docs/MCOS_High_Level_Plan.md` — §1 Project Overview: added paragraph noting MCOS builds on production HSTL tools with reference to §11
- `docs/MCOS_High_Level_Plan.md` — §4 Objects module: media attachment bullet updated to note IPTC/EXIF embedding via ExifTool
- `docs/MCOS_High_Level_Plan.md` — §6 Technology Overview: ExifTool added as a component for image metadata embedding
- `docs/MCOS_High_Level_Plan.md` — §7 Phase 5 Data Migration: CSV/Excel import bullet notes HSTL pipeline experience; batch image import bullet notes IPTC metadata extraction
- `docs/MCOS_High_Level_Plan.md` — old §11 Glossary renumbered to §12; version bumped to 0.8
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — §4 Phase 2 section renamed from "Locations, Loans & Donors" to "Members, Donors, Locations & Loans"; Members/Comms bullet moved from Phase 3 (Operations Modules) into Phase 2; Phase 0 schema reference updated to match new Phase 2 scope
- `docs/MCOS_Documentation_Plan.md` — Phase 2 user guide renamed from "User Guide: Locations, Loans & Donors" to "User Guide: Members, Donors, Locations & Loans"; Phase 3 user guide reduced from "Members & Inventory" to "Inventory" only
...
### Added
- `docs/MCOS_Documentation_Plan.md` — Test Plan added as a Phase 1 planned document: test framework and tooling, unit/integration/end-to-end coverage goals, how to run locally, conventions for new tests, CI execution

### Changed
- `docs/MCOS_Documentation_Plan.md` — Deployment Guide description updated: Docker Compose option replaced with guided setup option; backup language made specific (automated daily schedule and off-site copy instructions)
- `docs/MCOS_Documentation_Plan.md` — System Architecture Document description simplified; jargon removed; contributor framing clarified
- `docs/MCOS_Documentation_Plan.md` — Planned count corrected 10 → 11; version bumped to 0.4
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — six critique points addressed; version bumped to 0.7:
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — added GitHub repository paragraph to §8 Current Status: URL, plain-language explanation of GitHub's role in open source development, and note that no account is needed to browse or read documentation; version bumped to 0.6
...
- `README.md` — seven critique points addressed:
  - Opening sentence changed from "MCOS will be" to "MCOS is" — present tense is standard for GitHub READMEs
...
### Changed
- `docs/MCOS_Documentation_Plan.md` — seven critique points addressed:
...
### Added
- `docs/MCOS_Documentation_Plan.md` — tracks all project documentation: 6 complete, 10 planned across Phases 1–5; includes Deployment Guide, System Architecture Document, Administrator Guide, User Guides, IA Publisher Guide, Contributing Guide, Extension Developer Guide, and Data Migration Guide
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — seven critique points addressed:
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — nine critique points addressed:
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — removed WCAG entry from glossary; not referenced in the document body and not relevant to this audience
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — removed "IMLS and foundation grant applications leveraging open access mission" from Phase 4 deliverables; vague aspirational boilerplate for an audience that knows the grant landscape; "Grant Friendly" principle in §2 carries that message adequately
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — v0.2 → v0.3; Internet Archive publishing moved from standalone Phase 3 into Phase 1 as a core capability:
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — "Accessible by Design" principle reworded to "UI will be cross-platform and standards-based"; removed WCAG 2.1 AA reference (web standard, not applicable to a desktop app; jargon for this audience)
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — removed §9 Advisory Board (group is informal and dynamic; no need to document it in the plan); renumbered Competitive Landscape, Risks, and Glossary as §9–11
...
### Changed
- `docs/MCOS_High_Level_Plan.md` — major revision (v0.1 → v0.2) to target a senior museum director and archivist advisory board audience:
  - Added "The problem MCOS addresses" framing paragraph to §1
  - Added "What these institutions have in common" to §3 (Target Institutions)
  - Restructured §4 as "What MCOS Will Do" — reframed from module names to collections-operations capabilities
  - **Added §5 Standards Alignment** — SPECTRUM, Dublin Core, LIDO, AAM Core Standards, IMLS Data Stewardship; how MCOS aligns with each
...
### Removed
- `docs/MCOS_High_Level_Plan.html` — original LibreOffice export; superseded by `docs/MCOS_High_Level_Plan.md` which is the living document going forward
...
- `docs/MCOS_High_Level_Plan.md` — clarified deployment model in §4.2:
