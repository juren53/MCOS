...
_High Level Project Plan — Draft Version 0.25 — 2026-06-22-0606_
...
**Community input through GitHub Issues and Discussions:** Museum professionals are not passive end users in a project like MCOS — their day-to-day experience with collections work is exactly the knowledge that shapes a system worth using. GitHub Issues is the primary channel for that input: a registrar who spots a missing field, a volunteer who finds installation confusing, or a developer who identifies a bug can open an issue directly in the repository, where it enters a tracked queue visible to everyone. Feature requests are handled the same way — an institution that needs a capability MCOS doesn't yet have can make the case in an issue, and the broader community can weigh in on priority and design before any code is written.

GitHub Discussions is a separate space for open-ended conversation that doesn't fit neatly into the issue format — questions about whether MCOS is the right fit for a particular institution, suggestions about workflow design before requirements are firm, or broader questions about how field names should map to a standard. Discussions are organized by category and are searchable, so a new institution evaluating MCOS can read through what others have already asked and answered without duplicating the conversation. Together, Issues and Discussions ensure that the people who will actually use MCOS — small museum staff, registrars, trained volunteers, and the developers who support them — have a direct, low-friction way to shape the project's direction from the earliest phases of development.
